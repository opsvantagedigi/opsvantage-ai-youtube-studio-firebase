...
export const generateMonetisationPlanFlow = defineFlow(
  ...
  async (input) => {
    ...
    if (monetisationProfile) {
      // Limit the number of affiliate programs and digital products to prevent large inputs
      const limitedAffiliatePrograms = Array.isArray(monetisationProfile.affiliatePrograms)
        ? monetisationProfile.affiliatePrograms.slice(0, 10) // Limit to first 10 programs
        : [];

      const limitedDigitalProducts = Array.isArray(monetisationProfile.digitalProducts)
        ? monetisationProfile.digitalProducts.slice(0, 10) // Limit to first 10 products
        : [];

      monetisationPrompt += `The creator has these affiliate programs: ${JSON.stringify(limitedAffiliatePrograms)}. `;
      monetisationPrompt += `They offer these digital products: ${JSON.stringify(limitedDigitalProducts)}. `;
    }
