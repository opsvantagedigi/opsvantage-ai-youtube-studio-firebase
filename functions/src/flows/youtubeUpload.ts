...
export const uploadToYouTubeFlow = defineFlow(
  ...
  async (input) => {
    ...
    try {
      // Limit the script text to prevent exceeding input length limits
      const maxScriptLength = 500;
      const scriptPreview = scriptData?.scriptText
        ? scriptData.scriptText.length > maxScriptLength
          ? scriptData.scriptText.substring(0, maxScriptLength) + '...'
      ...
      const optimizationPrompt = `
        ...
        CURRENT DESCRIPTION: ${metadata.description.substring(0, 1000)}...  // Limit description preview
        CURRENT TAGS: ${metadata.tags.slice(0, 20).join(', ')}  // Limit number of tags shown

        PROJECT NICHE: ${projectData.niche}
        TARGET AUDIENCE: ${projectData.targetAudience}
        VIDEO SCRIPT: ${scriptPreview}
