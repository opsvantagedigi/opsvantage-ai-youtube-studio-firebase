...
export const runGrowthAnalyticsFlow = defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      // Limit the amount of data sent to prevent exceeding input length limits

      // Summarize channel stats to essential metrics only
      const essentialChannelStats = {
        subscriberCount: channelAnalytics.channelStats.subscriberCount,
        viewCount: channelAnalytics.channelStats.viewCount,
        videoCount: channelAnalytics.channelStats.videoCount,
        averageViewsPerVideo: channelAnalytics.channelStats.averageViewsPerVideo,
      };

      // Limit video analytics to top 5 performing videos and summarize key metrics
      const topPerformingVideos = videoAnalytics
        .sort((a: any, b: any) => b.analytics?.viewCount - a.analytics?.viewCount || 0)
        .slice(0, 5) // Only take top 5 videos
        .map(({ title, analytics }: { title: string, analytics: any }) => ({
      ...
      const analyticsPrompt = `
        ...
        Channel Stats: ${JSON.stringify(essentialChannelStats)}

        Top 5 Performing Videos: ${JSON.stringify(topPerformingVideos)}
