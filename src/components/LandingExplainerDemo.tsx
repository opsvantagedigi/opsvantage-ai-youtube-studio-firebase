...
export default function LandingExplainerDemo() {
...
export function LandingExplainerDemo() {
  ...
  async function generate() {
    ...
    setOutput("");
    try {
      const res = await fetch('/api/landing-explain', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: input, audience }) });
      if (!res.ok) {
        setOutput('Unable to generate explanation (demo mode)');
      } else {
        const json = await res.json();
